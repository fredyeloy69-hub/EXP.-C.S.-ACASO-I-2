...
export function generarReportePorArea(areaNombre, carpetasDelArea, opts = {}) {
  ...
  const nombreArchivo = `Reporte_${sufijoFiltroArchivo(tipoFiltro)}_${areaNombre.replace(/[^a-zA-Z0-9]+/g, "_")}_${new Date().toISOString().slice(0, 10)}.pdf`;
...
export function generarReporteConsolidadoGlobal(datosEntrada, opts = {}) {
  ...
  const nombreArchivo = `Reporte_${sufijoGlobal}_Consolidado_${new Date().toISOString().slice(0, 10)}.pdf`;
  doc.save(nombreArchivo);
}
