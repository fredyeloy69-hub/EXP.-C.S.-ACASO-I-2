...
/**
 * Genera y descarga un Excel (.xlsx) con la cantidad de hojas de cada
 * formato (A4, A3, A2, A1, A0) que hacen falta imprimir, por especialidad y
 * en total — para calcular cuánto material comprar.
 *
 * El formato de cada plano se detecta leyendo el nombre del archivo (ej.
 * "DIE-03 A2.pdf" → A2). Los planos que no traen el formato en el nombre
 * quedan en la columna "SIN IDENTIFICAR", y se listan uno por uno en una
 * segunda hoja para revisar a mano — nunca se adivina el formato.
 *
 * @param {string} areaNombre - nombre del área a exportar
 * @param {Array} carpetasDelArea - carpetas (ya filtradas a esa área), cada
 *   una con su array `archivos` ([{ nombre, tipo }, ...]) tal como lo guarda syncEngine.js
 */
export async function generarListaMaterialImpresion(areaNombre, carpetasDelArea) {
  ...
  // --- Recorrer todas las carpetas del área y clasificar cada archivo ---
  const porEspecialidad = {}; // { especialidad: { A4, A3, A2, A1, A0, sinIdentificar } }
  const totalGeneral = { A4: 0, A3: 0, A2: 0, A1: 0, A0: 0, sinIdentificar: 0 };
  const listaSinIdentificar = []; // { especialidad, nombre, ruta }

  function sumar(especialidad, formato) {
    if (!porEspecialidad[especialidad]) {
      porEspecialidad[especialidad] = { A4: 0, A3: 0, A2: 0, A1: 0, A0: 0, sinIdentificar: 0 };
    }
    porEspecialidad[especialidad][formato]++;
    totalGeneral[formato]++;
  }
  ...
  for (const c of carpetasDelArea) {
    ...
    for (const archivo of c.archivos || []) {
      const esCandidatoPlano = archivo.tipo === "dwg" || (archivo.tipo === "pdf" && esPlanos);

      if (esCandidatoPlano) {
        const formato = extraerFormatoDeNombre(archivo.nombre);
        if (formato) {
          sumar(especialidad, formato);
        } else {
          if (!porEspecialidad[especialidad]) {
            porEspecialidad[especialidad] = { A4: 0, A3: 0, A2: 0, A1: 0, A0: 0, sinIdentificar: 0 };
          }
          porEspecialidad[especialidad].sinIdentificar++;
          totalGeneral.sinIdentificar++;
          listaSinIdentificar.push({ especialidad, nombre: archivo.nombre, ruta: c.ruta || c.nombre });
        }
      } else if (archivo.tipo === "pdf" || archivo.tipo === "word" || archivo.tipo === "excel") {
        sumar(especialidad, "A4");
      }
      // rvt, modelo, respaldo, basura, otro → no cuentan como material a imprimir
    }
  }
  ...
  sheet.getCell("A4").value =
    `Generado: ${new Date().toLocaleString("es-PE")}  ·  el formato de cada plano se detecta por su nombre de archivo (ej. "...A2.pdf") — los que no lo traen quedan en "SIN IDENTIFICAR" (ver hoja 2)`;
  sheet.getCell("A4").font = { italic: true, size: 8.5, color: { argb: "FF666666" } };
  ...
  const FILA_ENCABEZADO = 6;
  const filaEncabezado = sheet.getRow(FILA_ENCABEZADO);
  filaEncabezado.values = ["ESPECIALIDAD", "A4", "A3", "A2", "A1", "A0", "SIN IDENTIFICAR", "TOTAL"];
  filaEncabezado.eachCell((cell) => {
    cell.font = { bold: true, color: { argb: "FFFFFFFF" } };
    cell.fill = { type: "pattern", pattern: "solid", fgColor: { argb: AZUL_PETROLEO } };
    cell.alignment = { vertical: "middle", horizontal: "center" };
  ...
  for (const esp of especialidadesOrdenadas) {
    const v = porEspecialidad[esp];
    const total = v.A4 + v.A3 + v.A2 + v.A1 + v.A0 + v.sinIdentificar;
    const fila = sheet.addRow([esp, v.A4, v.A3, v.A2, v.A1, v.A0, v.sinIdentificar, total]);
    fila.getCell(1).alignment = { horizontal: "left", vertical: "middle" };
    for (let col = 2; col <= 8; col++) {
      fila.getCell(col).alignment = { horizontal: "center", vertical: "middle" };
    }
    fila.getCell(7).font = { color: { argb: v.sinIdentificar > 0 ? "FFC0392B" : "FF999999" }, bold: v.sinIdentificar > 0 };
  ...
  const totalFinal =
    totalGeneral.A4 + totalGeneral.A3 + totalGeneral.A2 + totalGeneral.A1 + totalGeneral.A0 + totalGeneral.sinIdentificar;
  ...
  // --- Hoja 2: lista de archivos sin formato identificado, para revisar a mano ---
  if (listaSinIdentificar.length > 0) {
    const sheet2 = workbook.addWorksheet("Sin identificar");
    sheet2.columns = [{ width: 30 }, { width: 55 }, { width: 70 }];
    sheet2.addRow(["ESPECIALIDAD", "ARCHIVO", "RUTA"]).eachCell((cell) => {
      cell.font = { bold: true, color: { argb: "FFFFFFFF" } };
      cell.fill = { type: "pattern", pattern: "solid", fgColor: { argb: AZUL_PETROLEO } };
      cell.alignment = { vertical: "middle", horizontal: "center" };
    });
    for (const item of listaSinIdentificar.sort((a, b) => comparaNatural(a.especialidad, b.especialidad))) {
      sheet2.addRow([item.especialidad, item.nombre, item.ruta]);
    }
