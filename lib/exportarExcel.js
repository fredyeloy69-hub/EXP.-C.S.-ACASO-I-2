...
export async function generarListaMaterialImpresion(areaNombre, carpetasDelArea) {
  ...
  for (const esp of especialidadesOrdenadas) {
    ...
    const filaEsp = sheet.addRow([esp, v.A4, v.A3, v.A2, v.A1, v.A0, v.sinIdentificar, total]);
    ...
    for (const sub of subcarpetasOrdenadas) {
      ...
      const totalSub = s.A4 + s.A3 + s.A2 + s.A1 + s.A0 + s.sinIdentificar;
      const filaSub = sheet.addRow([sub, s.A4, s.A3, s.A2, s.A1, s.A0, s.sinIdentificar, totalSub]);
      ...
      filaSub.getCell(8).font = { bold: true };
