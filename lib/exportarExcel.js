...
// natural, respetando números), y por cada nodo extrae el número y la
// descripción reales desde su propio nombre — no se inventa numeración.
function numerarArbolSeparadores(nodo, resultado) {
  const claves = Object.keys(nodo).sort(comparaNaturalSep);
  for (const clave of claves) {
    const { numero, descripcion } = extraerNumeroYDescripcion(clave);
    resultado.push({ numero, nombre: descripcion });
    numerarArbolSeparadores(nodo[clave], resultado);
  }
}
