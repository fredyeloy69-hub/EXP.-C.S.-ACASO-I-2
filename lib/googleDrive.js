...
export async function contarPaginasPdf(fileId) {
  try {
    ...
    // Límite de tiempo por archivo — si un PDF en particular tarda demasiado
    // en descargar (uno muy pesado, o un problema de red puntual), que no se
    // coma todo el presupuesto de 10 segundos que da Vercel por sincronización.
    // Como todos los PDFs de la corrida se descargan EN PARALELO, este límite
    // no se suma entre archivos — el tiempo total depende del más lento, no
    // de la cantidad, así que se puede dar un margen generoso por archivo.
