...
export async function runSync() {
  ...
  const LIMITE_PAGINAS_POR_SYNC = 10;
  let contadosEnEstaCorrida = 0;

  async function obtenerPaginas(f) {
    ...
    // Límite de cuántos PDFs nuevos/cambiados se descargan y cuentan EN ESTA
    // sincronización — en el plan Hobby de Vercel, cada función se corta a
    // los 10 segundos, y descargar+abrir muchos PDFs se pasa fácil de eso.
    // Los que no alcanzan a contarse esta vez quedan en null (se ven como
    // "no se pudo leer" en el reporte) y se cuentan solos en la SIGUIENTE
    // sincronización — dale a "Sincronizar ahora" un par de veces seguidas
    // la primera vez, hasta que ya no falte ninguno.
    if (contadosEnEstaCorrida >= LIMITE_PAGINAS_POR_SYNC) {
      return null;
    }
    contadosEnEstaCorrida++;
    return await contarPaginasPdf(f.id);
  }
